...
# Define exceptions for unit tests and backward compatibility
class SCMAuthenticationError(Exception):
    """Authentication error from SCM API."""
    def __str__(self):
        return "Auth failed" if len(self.args) == 0 else self.args[0]

class SCMResourceNotFoundError(Exception):
...
class SCMBadRequestError(Exception):
    """Bad request error from SCM API."""
    pass

class SCMNotFoundError(Exception):
