...
try:
    ...
    from scm.exceptions import APIError as RealAPIError
    from scm.exceptions import AuthenticationError as RealAuthenticationError
    from scm.exceptions import ResourceNotFoundError as RealResourceNotFoundError
...
def get_scm_client(module):
    ...
    if not api_key:
        ...
        module.fail_json(
            msg="API key is required for SCM authentication. Provide via api_key parameter or SCM_API_KEY environment variable."
        )
    ...
    if not api_url:
        module.fail_json(
            msg="API URL is required for SCM authentication. Provide via api_url parameter or SCM_API_URL environment variable."
        )

    try:
        return SCMClient(api_key=api_key, base_url=api_url)
...
def handle_scm_error(module, error):
    ...
    if error.__class__.__name__ == "SCMAuthenticationError":
        msg = "Authentication error: Auth failed"
