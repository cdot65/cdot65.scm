...
from ansible_collections.cdot65.scm.plugins.module_utils.scm import get_scm_client
from ansible_collections.cdot65.scm.plugins.module_utils.scm import handle_scm_error
from ansible_collections.cdot65.scm.plugins.module_utils.scm import is_resource_exists
from ansible_collections.cdot65.scm.plugins.module_utils.scm import scm_argument_spec
...
def test_get_scm_client_with_params(mock_scm_client):
    ...
    module = MagicMock()
    module.params = {"api_key": "test_key", "api_url": "https://test.api.url"}
...
def test_get_scm_client_with_env_vars(mock_scm_client):
    ...
    module = MagicMock()
    module.params = {"api_key": None, "api_url": None}

    with patch.dict(os.environ, {"SCM_API_KEY": "env_test_key", "SCM_API_URL": "https://env.test.api.url"}):
        client = get_scm_client(module)

    mock_scm_client.assert_called_once_with(api_key="env_test_key", base_url="https://env.test.api.url")
    assert client is not None
...
def test_get_scm_client_missing_api_key(mock_scm_client):
    ...
    module = MagicMock()
    module.params = {"api_key": None, "api_url": "https://test.api.url"}
...
def test_is_resource_exists(mock_not_found):
    ...
    client.folder.list.return_value = [{"id": "123", "name": "test_folder"}, {"id": "456", "name": "other_folder"}]
